...
- For handled operational failures (soft-fail) that should be supportable/debuggable, write to `live_logs/error.log` *from the controller* using shared helpers while also setting post-close StatusBar intent:
...
- UI failures are logged to `live_logs/error.log`.
