...
- Reset Pay: `keyFastBtnrstpay` focuses `tenderValLineEdit`, clears it, and leaves focus there when tender is enabled/editable. This intentionally disables Pay until the cashier enters a valid tender value again.
...
- Make keypad Tab local: keypad Tab ignores Qt's tab chain and instead manually cycles focus through a fixed, keypad-only list of payment-panel widgets. This ensures the keypad's Tab navigation stays inside the payment panel and does not jump to other UI sections.
