...
The main window footer watches `log/error.log` through
...
including database setup such as `ensure_cash_outflows_table()`. If the database
path cannot be opened in that test context, the existing startup error handling
can write entries such as:

```text
Cash outflows table ensure failed: unable to open database file
```

These entries are generated by normal startup code, not manually by the status
footer. If they are only test artifacts, they can be cleared using the footer
Clear button.
...
- `Documentation/status_footer.md` for main footer error-log indicator, export, and clear behavior
