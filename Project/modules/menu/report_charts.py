...
from PyQt5.QtWidgets import QApplication, QDialog, QFrame, QGridLayout, QHBoxLayout, QLabel, QScrollArea, QSizePolicy, QVBoxLayout, QWidget
...
def _page_number_label(current: int, total: int = 3) -> QLabel:
    label = QLabel(f'{int(current)}/{int(total)}')
...
class _ChartReportCanvasPage1(QWidget):
    ...
    def __init__(self, report_data: Any):
        ...
        title_row.addWidget(_page_number_label(1))
...
class _ChartReportCanvasPage2(QWidget):
    ...
    def __init__(self, report_data: Any):
        ...
        layout.setContentsMargins(16, 16, 16, 16)
        layout.setSpacing(14)
        layout.addWidget(_page_number_label(2), alignment=Qt.AlignRight)
...
class _ChartReportCanvasPage3(QWidget):
    """Page 3: Top categories chart only."""
    def __init__(self, report_data: Any):
        super().__init__()
        self.setObjectName('chartReportCanvasPage3')
        self.setStyleSheet('QWidget#chartReportCanvasPage3 { background: #FFFFFF; }')
        payload = _safe_report(report_data)
        top_categories = payload.get('top_categories_by_sales_day') or []

        layout = QVBoxLayout(self)
        layout.setContentsMargins(16, 16, 16, 16)
        layout.setSpacing(14)
        layout.addWidget(_page_number_label(3), alignment=Qt.AlignRight)
        layout.addWidget(
            _ChartCard(
