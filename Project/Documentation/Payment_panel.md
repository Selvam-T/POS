...
- **Status priority**: validation error > (cash>0 and tender<cash) > (unalloc<0) > clear. Under-allocation is communicated by `unallocValPayLabel`, its highlighted frame, payment placeholders, and the disabled Pay button rather than a repetitive status message.
