...
class _ReportTextHighlighter(QSyntaxHighlighter):
    ...
    def highlightBlock(self, text: str) -> None:
        ...
        if text in ('SALES AUDIT REPORT', 'SALES INSIGHTS REPORT', 'INACTIVE PRODUCTS REPORT'):
...
def _format_summary_report_text(report: dict) -> tuple[str, set[str], set[str], set[str]]:
    ...
    lines.extend(['', '2. Average Hourly Revenue', '-' * 70])
    if sales_by_hour:
    ...
    _append_group_section('Top 5 Products by Hourly Volume Trend', top_qty_by_hour, 3)
    _append_group_section('Top 5 Products by Hourly Revenue Trend', top_sales_by_hour, 4)
    ...
    _append_day_section('Top 10 Products By Average Daily Volume', top_qty_day, 5)
    _append_day_section('Top 10 Products By Average Daily Revenue', top_sales_day, 6)
