...
Assumed layout:
  ...
  POS/db/Anumani.db      <- database lives here by default

Env overrides (optional):
  ADMIN_DB_PATH       Absolute path to the SQLite DB file (overrides derived path)
  ADMIN_DB_FILENAME   Defaults to 'Anumani.db'
  ADMIN_EXPORT_DIR    Absolute path for CSV exports (defaults to <this>/data)
"""
import os

CONFIG_DIR = os.path.abspath(os.path.dirname(__file__))  # .../POS/Database_admin/config
BASE_DIR = os.path.dirname(CONFIG_DIR)                   # .../POS/Database_admin
