...
def test_product_replacement_rejects_unknown_category_before_delete(
    tmp_path: Path,
) -> None:
    db_path = tmp_path / "working.db"
    csv_path = tmp_path / "products.csv"
    ...
    product = _product("P1", "First Product")
    product["category"] = "Missing Category"
    _write_products(csv_path, [product])

    with pytest.raises(
        ValueError,
        match="Product categories are absent from Category: Missing Category",
    ):
        replace_products(
            db_file=db_path,
            csv_file=csv_path,
